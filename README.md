...
- The solution is to use a dedicated level shifter which has internal active components to shift the signal with minimal distortion. Unfortunately, most of level shifter ICs are found online which I was unwilling to order. Alternatively wouldnt have to level shift the signal at all if you powered the multiplexer in dual supply mode. However, this would require a negative voltage rail which my DC wall adapter does not provide. Trying to generate my own negative voltage rail would be better suited for its own dedicated project.
